...
use crate::{
    models,
    vision_models::gemma3::{config::Gemma3TextConfig, text::TextModel as Gemma3TextModel},
...
impl NormalLoaderType {
    pub fn from_causal_lm_name(name: &str) -> Result<Self> {
        match name {
            ...
            "Gemma3ForCausalLM" => Ok(Self::Gemma3Text),
            other => anyhow::bail!(
...
impl FromStr for NormalLoaderType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            "gemma3_text" => Ok(Self::Gemma3Text),
            a => Err(format!("Unknown architecture `{a}`. Possible architectures: `mistral`, `gemma`, `mixtral`, `llama`, `phi2`, `phi3`, `qwen2`, `gemma2`, `starcoder2`, `phi3.5moe`, `deepseekv2`, `deepseekv3`, `qwen3`, `glm4`, `glm4moelite`, `glm4moe`, `qwen3moe`, `smollm3`, `granitemoehybrid`, `gpt_oss`, `qwen3next`, `gemma3_text`.")),
        }
    }
}
...
impl Display for NormalLoaderType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Gemma3Text => write!(f, "gemma3_text"),
        }
    }
}
...
impl AutoNormalLoader {
    fn get_loader(config: &str) -> Result<Box<dyn NormalModelLoader>> {
        ...
        match tp {
            ...
            NormalLoaderType::Gemma3Text => Ok(Box::new(Gemma3TextLoader)),
        }
    }
}
...
/// [`NormalLoader`] for a text-only Gemma3 model.
///
/// [`NormalLoader`]: https://docs.rs/mistralrs/latest/mistralrs/struct.NormalLoader.html
pub struct Gemma3TextLoader;

impl NormalModelLoader for Gemma3TextLoader {
    fn load(
        &self,
        config: &str,
        vb: ShardedVarBuilder,
        ...
        Ok(Box::new(Gemma3TextModel::new(
            &cfg,
            vb,
            self.is_gptx(config)?,
            normal_loading_metadata,
    ...
    fn load_xlora(
        ...
        anyhow::bail!("XLoRA is not supported for Gemma3 text-only models.")
    }
    fn is_gptx(&self, _: &str) -> Result<bool> {
        Ok(true)
    }
...
impl IsqModelLoader for Gemma3TextLoader {
    fn isq_layer_regexes(&self, _config: &str) -> Result<Vec<Regex>> {
        Ok(vec![
            Regex::new(r"lm_head\.(weight|bias)$")?,
            // Attention
...
impl DeviceMappedModelLoader for Gemma3TextLoader {
    fn mapped_max_act_size_elems(
        &self,
        config: &str,
        params: &AutoDeviceMapParams,
